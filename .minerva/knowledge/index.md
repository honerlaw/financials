...
- [[016-decision-daily-digest-notifier]] — Threshold budget alerts replaced by one 7am daily digest (budget status + every account balance) — dispatched only from run_daily_sync so a manual sync never texts, deduped per (sent_date, recipient), and scheduled in APP_TIMEZONE because the container is UTC.
